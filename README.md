...
在蒙特卡洛项目中，state 的定义不是很合理，所以在本次项目中，我们尝试重新定义 state.
...
在此项目临近结束，补充 readme 的时候，我们才突然想到，多个连续动作的截图组合在一起，可以认为是一个视频，业界似乎也有视频分类模型，但这次来不及尝试了，代码都写完了。。。下次吧，也不知道我们的破显卡能不能带得动。
...
到游戏里面尝试了这个分类模型，发现分类 0 经常会被误识别到其它分类里面去（分类 4 是重灾区），原因是 boss 的其它招式的部分动作确实与上面定义的四个招式中的部分动作有点相似。

为了解决这个问题，我们悟出了 `信号强度` 的概念：同一招式中连续多个帧的分类应该是相同的，在预测时，连续帧相同分类出现的次数可以称为`信号强度`，信号越强，表示对该招式的预测越准确。
稍微尝试了一下，发现只需要有连续三帧预测出来的分类相同，就可以显著缓解上述招式误识别的问题。

总结一下，我们做的其实是如下数据的转化：

实时截图 image -> 分类 class(不准确) -> 状态 state(相对准确) -> action(需要探索) 

看起来，似乎只要针对各个 state 定义一些处理规则，就可以应对 boss 的各种招式了，但有些 action 也值得探索一下，比如识破之后，出刀一次还是两次，抑或是三次四次五次。人类玩家在连续出刀的过程中可以实时观察 boss 状态来决定是否中止连招，但程序却需要事先明确连续出刀几次，因为程序发起的连招动作目前还无法在过程中随时中止，必须全部打完才能换下一个动作。

有没有一种在做 grid search 的感觉。

最后，定义了两个新的 state：5 与 6 与处理方法：
...
- X图片y按键 分类项目 https://github.com/XuLvXiu/sekiro_classifier_ai
